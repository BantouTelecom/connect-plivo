...
module.exports = function(client) {
  ...
  _.extend(Call, {
     ...
     * @param {string,Array,object} answerOrParams A url or route array to be requested by Plivo when the call is answered, or a object of outbound call parameters.
     * @param {function(err, result)} [cb] Callback
     * @return {ApiClient.Call}
     * @memberOf ApiClient.Call
     * @see <a target="_blank" href="http://www.plivo.com/docs/api/call/#outbound">http://www.plivo.com/docs/api/call/#outbound</a>
     */
    outbound: function(from, to, answerOrParams, cb) {
      var params = {
        from: from,
        to: to
      };

      params.answer_url = client._getCallbackUrl(answerOrParams.answer_url || answerOrParams);

      if (typeof answerOrParams === 'object') {
        // delete the answer_url property before extending over params, so we don't undo what we just did above
        delete answerOrParams.answer_url;

        _.extend(params, answerOrParams);
      }

      // We don't typically care about the hangup, and don't want to raise the answer callback with the hangup, so > /dev/null
      if (client._getIgnoreHangupUrl && !params.hangup_url && params.ignoreHangup !== false) {
        params.hangup_url = client._getIgnoreHangupUrl();
      }

      client._post('Call', params, 200, cb);
